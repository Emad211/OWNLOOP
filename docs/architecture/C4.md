...
The sanitized diagnostics projection is observational and read-only. It joins only allowlisted process counters, validated redaction summaries, controlled Run/finalization/Evidence-gap facts, and verified Candidate-validation reports. Its JSON support bundle is generated in memory on demand and is not persisted as an artifact, row, Event, log, or browser-storage entry.
...
/diagnostics            sanitized process, redaction, Run, finalization, Evidence-gap, and validation-quality diagnostics
```
...
10. Local settings, privacy controls, and explicit deletion are validated without durable secrets or hidden schedulers.
