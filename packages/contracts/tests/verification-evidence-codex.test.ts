...
function observation(exitCode: number | null, status: "passed" | "failed" | "observed_without_exit_code") {
  return {
    observationIndex: 0,
...
describe("neutral Codex verification outcomes", () => {
  it.each([
    ...
    expect(VerificationCommandObservationV1Schema.parse(observation(exitCode, status))).toMatchObject({
  ...
  it("rejects a success claim when neutral completion carries a non-zero exit code", () => {
    expect(
      VerificationCommandObservationV1Schema.safeParse(observation(2, "passed")).success,
    ).toBe(false);
  });
});
